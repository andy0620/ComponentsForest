...
INT CMultipleCameraDlg::OnDrawPicture(dvpHandle handle, dvpStreamEvent event, void* pContext, dvpFrame* pFrame, void* pBuffer)
{
...
void CMultipleCameraDlg::InitCtrls1()
{
	...
	if (IsValidHandle(m_handle[0]))
	{
		...
		GetDlgItem(IDC_STATIC_OPEN1)->SetWindowText(CStringW(info.FriendlyName));
		GetDlgItem(IDC_EDIT_USER_ID1)->SetWindowText(CStringW(info.UserID));
	}
	else
	{
...
void CMultipleCameraDlg::InitCtrls2()
{
	...
	if (IsValidHandle(m_handle[1]))
	{
		...
		ASSERT(status == DVP_STATUS_OK);
		GetDlgItem(IDC_STATIC_OPEN2)->SetWindowText(CStringW(info.FriendlyName));
		GetDlgItem(IDC_EDIT_USER_ID2)->SetWindowText(CStringW(info.UserID));
	}
	else
	{
		// No device is opened at this time.
...
void CMultipleCameraDlg::InitCtrls3()
{
	...
	if (IsValidHandle(m_handle[2]))
	{
		...
		GetDlgItem(IDC_STATIC_OPEN3)->SetWindowText(CStringW(info.FriendlyName));
		GetDlgItem(IDC_EDIT_USER_ID3)->SetWindowText(CStringW(info.UserID));
	}
	else
	{
		// No device is opened at this time.
...
void CMultipleCameraDlg::InitCtrls4()
{
	...
	if (IsValidHandle(m_handle[3]))
	{
		...
		ASSERT(status == DVP_STATUS_OK);
		GetDlgItem(IDC_STATIC_OPEN4)->SetWindowText(CStringW(info.FriendlyName));
		GetDlgItem(IDC_EDIT_USER_ID4)->SetWindowText(CStringW(info.UserID));
	}
	else
	{
		// No device is opened at this time.
	...
	if (status == DVP_STATUS_OK)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			if (status == DVP_STATUS_OK)
			{
				...
				m_listDevices1.AddString(CStringW(m_info1[i].FriendlyName));
			}
		}
		...
		if (n > 0)
		{
			...
			SetDlgItemText(IDC_EDIT_USER_ID1, CStringW(m_info1[0].UserID));
		}
...
void CMultipleCameraDlg::OnBnClickedButtonScan2()
{
	...
	if (status == DVP_STATUS_OK)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			{
				// GUI need UNICODE,but the information acquired from camera is ANSI,so convert the character set from ANSI to UNICODE.
				m_listDevices2.AddString(CStringW(m_info2[i].FriendlyName));
			}
		}
		...
		if (n > 1)
		{
			...
			SetDlgItemText(IDC_EDIT_USER_ID2, CStringW(m_info2[1].UserID));
		}
...
void CMultipleCameraDlg::OnBnClickedButtonScan3()
{
	...
	if (status == DVP_STATUS_OK)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			{
				// GUI need UNICODE,but the information acquired from camera is ANSI,so convert the character set from ANSI to UNICODE.
				m_listDevices3.AddString(CStringW(m_info3[i].FriendlyName));
			}
		}
		...
		if (n > 2)
		{
			...
			SetDlgItemText(IDC_EDIT_USER_ID3, CStringW(m_info3[2].UserID));
		}
...
void CMultipleCameraDlg::OnBnClickedButtonScan4()
{
	...
	if (status == DVP_STATUS_OK)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			{
				// GUI need UNICODE,but the information acquired from camera is ANSI,so convert the character set from ANSI to UNICODE.
				m_listDevices4.AddString(CStringW(m_info4[i].FriendlyName));
			}
		}
		...
		if (n > 3)
		{
			...
			SetDlgItemText(IDC_EDIT_USER_ID4, CStringW(m_info4[3].UserID));
		}
...
void CMultipleCameraDlg::OnBnClickedButtonOpen1()
{
	...
	if (!IsValidHandle(m_handle[0]))
	{
		...
		if (strName != "")
		{
			...
			status = dvpOpenByName(CStringA(strName), OPEN_NORMAL, &m_handle[0]);
			if (status != DVP_STATUS_OK)
			{
			...
			else
			{
				...
				status = dvpRegisterStreamCallback(m_handle[0], OnDrawPicture, STREAM_EVENT_FRAME_THREAD, this);
				ASSERT(status == DVP_STATUS_OK);

				SetDlgItemTextA(GetSafeHwnd(), IDC_STATIC_OPEN1, CStringA(strName));
...
void CMultipleCameraDlg::OnBnClickedButtonOpen2()
{
	...
	if (!IsValidHandle(m_handle[1]))
	{
		...
		{
			// Open the pointed device by the selected FriendlyName.
			status = dvpOpenByName(CStringA(strName), OPEN_NORMAL, &m_handle[1]);
			if (status != DVP_STATUS_OK)
			{
			...
			else
			{
				...
				// If it needs to display images ,the user should register a callback function and finish the operation of drawing pictures in the registered callback function.
				// Note: Drawing pictures in the callback function maybe generate some delays for acquiring image data by the use of "dvpGetFrame".
				status = dvpRegisterStreamCallback(m_handle[1], OnDrawPicture, STREAM_EVENT_FRAME_THREAD, this);
				ASSERT(status == DVP_STATUS_OK);
				SetDlgItemTextA(GetSafeHwnd(), IDC_STATIC_OPEN2, CStringA(strName));
			}
		}
	}
	else
...
void CMultipleCameraDlg::OnBnClickedButtonOpen3()
{
	...
	if (!IsValidHandle(m_handle[2]))
	{
		...
		{
			// Open the pointed device by the selected FriendlyName.
			status = dvpOpenByName(CStringA(strName), OPEN_NORMAL, &m_handle[2]);
			if (status != DVP_STATUS_OK)
			{
			...
			else
			{
				...
				// If it needs to display images ,the user should register a callback function and finish the operation of drawing pictures in the registered callback function.
				// Note: Drawing pictures in the callback function maybe generate some delays for acquiring image data by the use of "dvpGetFrame".
				status = dvpRegisterStreamCallback(m_handle[2], OnDrawPicture, STREAM_EVENT_FRAME_THREAD, this);
				ASSERT(status == DVP_STATUS_OK);

				SetDlgItemTextA(GetSafeHwnd(), IDC_STATIC_OPEN3, CStringA(strName));
			}
		}
	}
	else
...
void CMultipleCameraDlg::OnBnClickedButtonOpen4()
{
	...
	if (!IsValidHandle(m_handle[3]))
	{
		...
		{
			// Open the pointed device by the selected FriendlyName.
			status = dvpOpenByName(CStringA(strName), OPEN_NORMAL, &m_handle[3]);
			if (status != DVP_STATUS_OK)
			{
			...
			else
			{
				...
				// If it needs to display images ,the user should register a callback function and finish the operation of drawing pictures in the registered callback function.
				// Note: Drawing pictures in the callback function maybe generate some delays for acquiring image data by the use of "dvpGetFrame".
				status = dvpRegisterStreamCallback(m_handle[3], OnDrawPicture, STREAM_EVENT_FRAME_THREAD, this);
				ASSERT(status == DVP_STATUS_OK);

				SetDlgItemTextA(GetSafeHwnd(), IDC_STATIC_OPEN4, CStringA(strName));
			}
		}
	}
	else
...
void CMultipleCameraDlg::UpdataCameraId(int n_pos, char* pFriendlyName, char* pUserID)
{
	...
	CString name;
	CString StrFriendlyName = CStringW(pFriendlyName);
	CString strUserID = CStringW(pUserID);
...
void CMultipleCameraDlg::OnBnClickedButtonSet1()
{
	if (IsValidHandle(m_handle[0]))
	{
		...
		dvpStatus status = dvpSetUserId(m_handle[0], CStringA(str.GetBuffer()), &Length);
		ASSERT(status == DVP_STATUS_OK);

		GetDlgItem(IDC_BUTTON_SAVE_BIND)->EnableWindow(TRUE);
		strcpy_s(m_info1[m_listDevices1.GetCurSel()].UserID, sizeof(m_info1[m_listDevices1.GetCurSel()].UserID), CStringA(str.GetBuffer()));
...
void CMultipleCameraDlg::OnBnClickedButtonSet2()
{
	if (IsValidHandle(m_handle[1]))
	{
		...
		// Set the user define name.
		dvpStatus status = dvpSetUserId(m_handle[1], CStringA(str.GetBuffer()), &Length);
		ASSERT(status == DVP_STATUS_OK);

		GetDlgItem(IDC_BUTTON_SAVE_BIND)->EnableWindow(TRUE);
		strcpy_s(m_info2[m_listDevices2.GetCurSel()].UserID, sizeof(m_info2[m_listDevices2.GetCurSel()].UserID), CStringA(str.GetBuffer()));
...
void CMultipleCameraDlg::OnBnClickedButtonSet3()
{
	if (IsValidHandle(m_handle[2]))
	{
		...
		// Set the user define name.
		dvpStatus status = dvpSetUserId(m_handle[2], CStringA(str.GetBuffer()), &Length);
		ASSERT(status == DVP_STATUS_OK);

		GetDlgItem(IDC_BUTTON_SAVE_BIND)->EnableWindow(TRUE);
		strcpy_s(m_info3[m_listDevices3.GetCurSel()].UserID, sizeof(m_info3[m_listDevices3.GetCurSel()].UserID), CStringA(str.GetBuffer()));
...
void CMultipleCameraDlg::OnBnClickedButtonSet4()
{
	if (IsValidHandle(m_handle[3]))
	{
		...
		// Set the user define name.
		dvpStatus status = dvpSetUserId(m_handle[3], CStringA(str.GetBuffer()), &Length);
		ASSERT(status == DVP_STATUS_OK);

		GetDlgItem(IDC_BUTTON_SAVE_BIND)->EnableWindow(TRUE);
		strcpy_s(m_info4[m_listDevices4.GetCurSel()].UserID, sizeof(m_info4[m_listDevices4.GetCurSel()].UserID), CStringA(str.GetBuffer()));
...
void CMultipleCameraDlg::OnBnClickedButtonLoadBind()
{
	...
	if (OpenByUserId(CStringA(strUserId0).GetString(), &m_handle[0]))
	{
		dvpStatus status = dvpRegisterStreamCallback(m_handle[0], OnDrawPicture, STREAM_EVENT_FRAME_THREAD, this);
		ASSERT(status == DVP_STATUS_OK);
	...
	if (OpenByUserId(CStringA(strUserId1).GetString(), &m_handle[1]))
	{
		dvpStatus status = dvpRegisterStreamCallback(m_handle[1], OnDrawPicture, STREAM_EVENT_FRAME_THREAD, this);
		ASSERT(status == DVP_STATUS_OK);
	...
	if (OpenByUserId(CStringA(strUserId2.GetBuffer()).GetString(), &m_handle[2]))
	{
		dvpStatus status = dvpRegisterStreamCallback(m_handle[2], OnDrawPicture, STREAM_EVENT_FRAME_THREAD, this);
		ASSERT(status == DVP_STATUS_OK);
	...
	if (OpenByUserId(CStringA(strUserId3).GetString(), &m_handle[3]))
	{
		dvpStatus status = dvpRegisterStreamCallback(m_handle[3], OnDrawPicture, STREAM_EVENT_FRAME_THREAD, this);
		ASSERT(status == DVP_STATUS_OK);
...
void CMultipleCameraDlg::OnCbnSelchangeComboDevices1()
{
	...
	{
		SetDlgItemText(IDC_EDIT_USER_ID1, CStringW(m_info1[m_listDevices1.GetCurSel()].UserID));
	}
	else
	{
...
void CMultipleCameraDlg::OnCbnSelchangeComboDevices2()
{
	...
	{
		SetDlgItemText(IDC_EDIT_USER_ID2, CStringW(m_info2[m_listDevices2.GetCurSel()].UserID));
	}
	else
	{
...
void CMultipleCameraDlg::OnCbnSelchangeComboDevices3()
{
	...
	{
		SetDlgItemText(IDC_EDIT_USER_ID3, CStringW(m_info3[m_listDevices3.GetCurSel()].UserID));
	}
	else
	{
...
void CMultipleCameraDlg::OnCbnSelchangeComboDevices4()
{
	...
	{
		SetDlgItemText(IDC_EDIT_USER_ID4, CStringW(m_info4[m_listDevices4.GetCurSel()].UserID));
	}
	else
	{
