...
void CImageAcquisitionDlg::UpdataPreWindowSize(int x, int y)
{
	if (IsValidHandle(m_handle))
	{
		...
		dvpInt32 x,y,w,h;
		status = dvpGetInt32Value(m_handle, V_OFFSET_X_I, &x, NULL);
		status = dvpGetInt32Value(m_handle, V_OFFSET_Y_I, &y, NULL);
		status = dvpGetInt32Value(m_handle, V_WIDTH_I, &w, NULL);
		status = dvpGetInt32Value(m_handle, V_HEIGHT_I, &h, NULL);
		roi.X = x;
		roi.Y = y;
		roi.W = w;
		roi.H = h;
...
void CImageAcquisitionDlg::UpdateControls()
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		status = dvpGetBoolValue(m_handle, V_TRIGGER_MODE_B, &bTriggerFlg);
		if (status == DVP_STATUS_OK)
		{
...
UINT CImageAcquisitionDlg::ImageThread()
{
	...
	while (WaitForSingleObject(m_evtExit, m_uWaitTimer) != WAIT_OBJECT_0)
	{
		...
		if (m_bSoftTrigger)
		{
			...
			status = dvpGetBoolValue(m_handle, V_TRIGGER_MODE_B, &bSoftTrigger);
			// ASSERT(status == DVP_STATUS_OK);
			if (bSoftTrigger)
			{
				//Trigger Frame
				status = dvpSetCommandValue(m_handle, "TriggerSoftware");
				if (status != DVP_STATUS_OK)
				{
	...
	if (IsValidHandle(m_handle))
	{
		...
		else
		{
			if (UpdateData(TRUE))
			{
				...
				status = dvpGetBoolValue(m_handle, V_TRIGGER_MODE_B, &bTrigerFlg);
				// Set as Trigger mode before starting video stream.
				if (status == DVP_STATUS_OK)
				{
					status = dvpSetBoolValue(m_handle, V_TRIGGER_MODE_B, m_bSoftTrigger ? TRUE : FALSE);
					if (status != DVP_STATUS_OK)
					{
