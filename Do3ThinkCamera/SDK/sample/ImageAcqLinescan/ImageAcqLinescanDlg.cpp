...
BOOL CImageAcqLinescanDlg::OnInitDialog()
{
	...
	m_cbxTrigType.AddString(CString("行触发"));
	m_cbxTrigType.AddString(CString("帧触发"));
	m_cbxTrigType.AddString(CString("连续模式"));
	m_cbxTrigType.AddString(CString("行+帧模式"));
...
void CImageAcqLinescanDlg::OnBnClickedButtonProperty()
{
	if (IsValidHandle(m_handle))
	{
		...
		bool TrigEnable;
		dvpStatus st =dvpGetBoolValue(m_handle, V_LINE_TRIG_ENABLE_B, &TrigEnable);
		if (st != DVP_STATUS_OK)
		{
			return;	
		}
		...
		bool pTriggerState;
		st = dvpGetBoolValue(m_handle, V_TRIGGER_MODE_B, &pTriggerState);
		if (st != DVP_STATUS_OK)
		{
			return;
		}
		...
		{
			//continuous mode index
			m_cbxTrigType.SetCurSel(2);
		}
		else
		{
			//line+frame mode index
...
void CImageAcqLinescanDlg::OnCbnSelchangeCbxtrigtype()
{
	...
	if (nSel == 0)   //line trigger
	{
		//Set line trigger to enable
		dvpStatus st = dvpSetBoolValue(m_handle, V_LINE_TRIG_ENABLE_B, true);
		//Set frame trigger to disable
		st = dvpSetBoolValue(m_handle, V_TRIGGER_MODE_B, false);


		GetDlgItem(IDC_BUTTON_SOFTTRIG)->EnableWindow(false);

	}
	else if (nSel == 1) //frame trigger
	{
		//Set line trigger to disable
		dvpStatus st = dvpSetBoolValue(m_handle, V_LINE_TRIG_ENABLE_B, false);
		//Set frame trigger to enable
		st = dvpSetBoolValue(m_handle, V_TRIGGER_MODE_B, true);

		GetDlgItem(IDC_BUTTON_SOFTTRIG)->EnableWindow(true);
	}
	else if (nSel == 2)  //continuous mode
	{
		//Set line trigger to disable
		dvpStatus st = dvpSetBoolValue(m_handle, V_LINE_TRIG_ENABLE_B, false);
		//Set frame trigger to disable
...
void CImageAcqLinescanDlg::OnBnClickedButtonApply()
{
	...
	float fExpose = (float)_ttof(strExpose);
	dvpStatus st = dvpSetFloatValue(m_handle, V_EXPOSURE_TIME_F, fExpose);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置曝光失败"), _T("Error"));
	}

	//Set gain value
	CString strGain;
	GetDlgItem(IDC_EDIT_GAIN)->GetWindowText(strGain);
	float fGain = (float)_ttof(strGain);
	st = dvpSetFloatValue(m_handle, V_GAIN_F, fGain);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置增益失败"), _T("Error"));
	}

	//Set trigger source
	CString strTrigSource;
	m_cbxTrigSource.GetWindowText(strTrigSource);
	st = dvpSetEnumValueByString(m_handle, V_LINE_TRIG_SOURCE_E, strTrigSource);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置触发源失败"), _T("Error"));
	}

	//Set trigger filtering
	CString strTrigFilter;
	GetDlgItem(IDC_EDIT_TRIGFILTER)->GetWindowText(strTrigFilter);
	float fTrigFilter = (float)_ttof(strTrigFilter);
	st = dvpSetFloatValue(m_handle, V_LINE_TRIG_FILTER_F, fTrigFilter);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置触发过滤失败"), _T("Error"));
	}

	//Set trigger delay
	CString strTrigDelay;
	GetDlgItem(IDC_EDIT_TRIGDELAY)->GetWindowText(strTrigDelay);
	float fTrigDelay = (float)_ttof(strTrigDelay);
	st = dvpSetFloatValue(m_handle, V_LINE_TRIG_DELAY_F, fTrigDelay);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置触发延时失败"), _T("Error"));
	}
	...
	int64_t nFreqPreDiv= _atoi64(strTrigFreqPreDiv);
	int64_t nFreqMult = _atoi64(strTrigFreqMult);
	int64_t nFreqDiv = _atoi64(strTrigFreqDiv);


	//Set prescaler
	st = dvpSetIntValue(m_handle, V_LINE_TRIG_FREQ_PRE_DIV_I, nFreqPreDiv);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置线触发预分频失败"), _T("Error"));
	}
	//Set multiplier  
	st = dvpSetIntValue(m_handle, V_LINE_TRIG_FREQ_MULT_I, nFreqMult);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置线触发倍频失败"), _T("Error"));
	}

	//Set frequency division
	st = dvpSetIntValue(m_handle, V_LINE_TRIG_FREQ_DIV_I, nFreqDiv);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置线触发分频失败"), _T("Error"));
	}

	//Set line frequency 
	CString strTrigLineRate;
	GetDlgItem(IDC_EDIT_LINERATE)->GetWindowText(strTrigLineRate);
	int nLine = _ttoi(strTrigLineRate);
	st = dvpSetInt32Value(m_handle, V_LINE_RATE_I, nLine);
	if (st != DVP_STATUS_OK)
	{
		MessageBox(_T("设置行频失败"), _T("Error"));
	}
...
void CImageAcqLinescanDlg::InitUI()
{
	if (IsValidHandle(m_handle))
	{
		if (m_cbxTrigSource.GetCount() <= 0)
		{
			...
			//Get trigger source
			dvpStatus st = dvpGetEnumValue(m_handle, V_LINE_TRIG_SOURCE_E, &nCurEnumValue, nSupportValue, &nSupportNum);
			...
			for (int i = 0; i < (int)nSupportNum; i++)
			{
				...
				//Get trigger source description information
				dvpEnumDescr stEnumDescr;
				dvpGetEnumDescr(m_handle, V_LINE_TRIG_SOURCE_E, i, &stEnumDescr);

				m_cbxTrigSource.AddString(stEnumDescr.szEnumName);
			}
		}
		//Get exposure value
		float fExpoTime;
		CString strExpoTime;
		if (dvpGetFloatValue(m_handle, V_EXPOSURE_TIME_F, &fExpoTime, NULL) == DVP_STATUS_OK)
		{
			CString strExpose;
			strExpose.Format(_T("%.3f"), fExpoTime);
		...
		else
		{
			...
			MessageBox(_T("获取曝光值失败"), _T("Error"));
		}


		//Get gain value
		float gain;
		if (dvpGetFloatValue(m_handle, V_GAIN_F, &gain, NULL) == DVP_STATUS_OK)
		{
			CString strGain;
		...
		else
		{
			...
			MessageBox(_T("获取增益值失败"), _T("Error"));

		}

		//Get trigger source
		char  LineTrigSource[128] = {0};
		dvpStatus st = dvpGetEnumValueByString(m_handle, V_LINE_TRIG_SOURCE_E, LineTrigSource);
		...
		{
			MessageBox(_T("获取触发源失败"), _T("Error"));
		}

		//Get scan direction
		bool ReverseScanDirection;
		st = dvpGetBoolValue(m_handle, V_REVERSE_SCAN_DIRECTION_B, &ReverseScanDirection);
		if (st == DVP_STATUS_OK)
		{
		...
		{
			MessageBox(_T("获取扫描方向失败"), _T("Error"));
		}

		//Get trigger filter
		float fLineTrigFilter = 0.0f;
		st = dvpGetFloatValue(m_handle, V_LINE_TRIG_FILTER_F, &fLineTrigFilter, NULL);

		if (st == DVP_STATUS_OK)
		{
			CString strLineTrigFilter;
			strLineTrigFilter.Format(_T("%f"), fLineTrigFilter);

			GetDlgItem(IDC_EDIT_TRIGFILTER)->SetWindowText(strLineTrigFilter);
		}
		else
		{
			GetDlgItem(IDC_EDIT_TRIGFILTER)->SetWindowText((LPCTSTR)"");
			MessageBox(_T("获取触发过滤失败"), _T("Error"));
		}

		//Get trigger delay
		float fLineTrigDelay = 0.0f;
		st = dvpGetFloatValue(m_handle, V_LINE_TRIG_DELAY_F, &fLineTrigDelay, NULL);

		if (st == DVP_STATUS_OK)
		{
			CString strLineTrigDelay;
			strLineTrigDelay.Format(_T("%f"), fLineTrigDelay);
			GetDlgItem(IDC_EDIT_TRIGDELAY)->SetWindowText(strLineTrigDelay);
		}
		else
		{
			GetDlgItem(IDC_EDIT_TRIGDELAY)->SetWindowText((LPCTSTR)"");
			MessageBox(_T("获取触发延时失败"), _T("Error"));
		}

		//Get line trigger prescaler
		int64_t nLinePreDiv;
		st = dvpGetIntValue(m_handle, V_LINE_TRIG_FREQ_PRE_DIV_I,&nLinePreDiv,NULL);
		if (st == DVP_STATUS_OK)
		{
			CString strPreDiv;
			strPreDiv.Format(_T("%lld"), nLinePreDiv);
		...
		else
		{
			...
			MessageBox(_T("获取线触发预分频失败"), _T("Error"));
		}

		//Get line trigger multiplier
		int64_t nLineMultDiv;
		st = dvpGetIntValue(m_handle, V_LINE_TRIG_FREQ_MULT_I, &nLineMultDiv, NULL);

		if(st == DVP_STATUS_OK)
		{
			CString strMult;
			strMult.Format(_T("%lld"), nLineMultDiv);
		...
		else
		{
			...
			MessageBox(_T("获取线触发倍频失败"), _T("Error"));
		}

		//Get line trigger frequency division
		int64_t nLineDiv;
		st = dvpGetIntValue(m_handle, V_LINE_TRIG_FREQ_DIV_I, &nLineDiv, NULL);
		if(st == DVP_STATUS_OK)
		{
			CString strDiv;
			strDiv.Format(_T("%lld"), nLineDiv);
		...
		else
		{
			...
			MessageBox(_T("获取线触发分频失败"), _T("Error"));
		}

		//Get line frequency
		int nLineRate;
		st = dvpGetInt32Value(m_handle, V_LINE_RATE_I, &nLineRate, NULL);

		if (st == DVP_STATUS_OK)
		{
			CString strLineRate;
			strLineRate.Format("%d", nLineRate);
			GetDlgItem(IDC_EDIT_LINERATE)->SetWindowText(strLineRate);
		}
		else
		{
			GetDlgItem(IDC_EDIT_LINERATE)->SetWindowText((LPCTSTR)"");
			MessageBox(_T("获取行频失败"), _T("Error"));
		}

		//Forward encoder count
		int nEncoderForwardCounter=0;
		st = dvpGetInt32Value(m_handle, V_ENCODER_FORWARD_COUNTER_I, &nEncoderForwardCounter, NULL);

		if(st == DVP_STATUS_OK)
		{
			CString strEncoderForwardCounter;
			strEncoderForwardCounter.Format("%d", nEncoderForwardCounter);
		...
		else
		{
			...
			MessageBox(_T("获取正向编码器计数失败"), _T("Error"));
		}

		//Reverse encoder count
		int nEncoderBackwardCounter = 0;
		st = dvpGetInt32Value(m_handle, V_ENCODER_BACKWARD_COUNTER_I, &nEncoderBackwardCounter, NULL);

		if(st == DVP_STATUS_OK)
		{
			CString strEncoderBackwardCounter;
			strEncoderBackwardCounter.Format("%d", nEncoderBackwardCounter);
		...
		else
		{
			...
			MessageBox(_T("获取反向编码器计数失败"), _T("Error"));
		}
	}
...
void CImageAcqLinescanDlg::OnBnClickedCheckForward()
{
	...
	if (nForward == 0)
	{
		dvpStatus st = dvpSetBoolValue(m_handle, V_REVERSE_SCAN_DIRECTION_B,false);
	}
	else
	{
		dvpStatus st = dvpSetBoolValue(m_handle, V_REVERSE_SCAN_DIRECTION_B,true);
	}
}
