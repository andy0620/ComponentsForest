	...
	void* pBuffer)
{
	...
	if (pDlg->m_bSaveImageFlg)
	{
		...
		dvpStatus status = dvpSavePicture(pFrame, pBuffer, CStringA(pDlg->m_ImagePath.GetBuffer()), 100);
		ASSERT(status == DVP_STATUS_OK);
...
void CBasicFunctionDlg::OnOK()
{
	if (IsValidHandle(m_handle))
	{
		// After clicking "OK" ,configured parameters will be save,default to the path "C:\ProgramData\DO3THINK\DVP2" if not specified.
...
void CBasicFunctionDlg::UpdateControls()
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		status = dvpGetEnumValueByString(m_handle, V_EXPOSURE_AUTO_E, szTmpExp);
		ASSERT(status == DVP_STATUS_OK);

		//auto exposure is closed
		if (strcmp(szTmpExp, "Disable") == 0)
		{
			UpdateData(FALSE);
		...
		else if ((strcmp(szTmpExp, "Continuous") == 0) || (strcmp(szTmpExp, "Once") == 0))   //auto exposure is opened
		{
			UpdateData(FALSE);
		...
		((CComboBox*)GetDlgItem(IDC_COMBO_AE_MODE))->SelectString(0,CStringW(szTmpExp));

		// Update the Anti-Flick control.
		dvpInt32 AntiFlick;
		status=dvpGetEnumValueEx(m_handle, V_ANTI_FLICK_E, NULL, &AntiFlick, NULL);
		//ASSERT(status == DVP_STATUS_OK);
		...
		float fExpoTime;
		CString strExpoTime;
		status = dvpGetFloatValue(m_handle, V_EXPOSURE_TIME_F, &fExpoTime, NULL);
		//ASSERT(status == DVP_STATUS_OK);
		...
		CString strGain;
		float fGain;
		status = dvpGetFloatValue(m_handle, V_GAIN_F, &fGain, NULL);
		//ASSERT(status == DVP_STATUS_OK);
...
void CBasicFunctionDlg::UpdateControlExposure()
{
	float fExpoTime;
	CString strValue;
	dvpFloatDescr ExpoTimeDescr;
	dvpStatus status;

	// Get the descriptive information about the exposure time.
	// Get the initial value of the exposure time.
	status = dvpGetFloatValue(m_handle, V_EXPOSURE_TIME_F, &fExpoTime, &ExpoTimeDescr);
	//ASSERT(status == DVP_STATUS_OK);
...
void CBasicFunctionDlg::UpdateControlGain()
{
	...
	status = dvpGetFloatValue(m_handle, V_GAIN_F, &fAnalogGain, &AnalogGainDescr);
...
void CBasicFunctionDlg::OnInitControlAeMode()
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		dvpStatus  status = dvpGetEnumValue(m_handle, V_EXPOSURE_AUTO_E, &nCurEnumValue, nSupportValue, &nSupportNum);
		...
		for (int i = 0; i < (int)nSupportNum; i++)
		{
			...
			dvpGetEnumDescr(m_handle, V_EXPOSURE_AUTO_E, i, &stEnumDescr);

			((CComboBox*)GetDlgItem(IDC_COMBO_AE_MODE))->AddString(CStringW(stEnumDescr.szEnumName));
		}

		((CComboBox*)GetDlgItem(IDC_COMBO_AE_MODE))->SetCurSel(nCurIndex);
	}
}
...
void CBasicFunctionDlg::OnInitControlFlick()
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		((CComboBox*)GetDlgItem(IDC_COMBO_FLICK))->InsertString(0, CStringW("ANTIFLICK_DISABLE"));
		((CComboBox*)GetDlgItem(IDC_COMBO_FLICK))->InsertString(1, CStringW("ANTIFLICK_50HZ"));
		((CComboBox*)GetDlgItem(IDC_COMBO_FLICK))->InsertString(2, CStringW("ANTIFLICK_60HZ"));

		// Set the current anti-flick status.
		((CComboBox*)GetDlgItem(IDC_COMBO_FLICK))->SetCurSel(0);
	}
}
	...
	if (status == DVP_STATUS_OK)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			if (status == DVP_STATUS_OK)
			{
				...
				{
					item = m_listDevices.AddString(CStringW(m_info[m_CamCount].FriendlyName));
				}
				else
				{
					...
					if (strlen(m_info[m_CamCount].UserID) == 0)
						continue;
					item = m_listDevices.AddString(CStringW(m_info[m_CamCount].UserID));
				}
				if (item != CB_ERR)
				{
...
void CBasicFunctionDlg::OnNMCustomdrawSliderExposureTime(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		fExpoTime = (float)((CSliderCtrl*)GetDlgItem(IDC_SLIDER_EXPOSURE_TIME))->GetPos();

		// Set the exposure time. 
		// Get the range of the exposure time,in order to avoid that the set value is out of range.
		status = dvpGetFloatValue(m_handle, V_EXPOSURE_TIME_F, NULL, &ExpDescr);
		...
		// Set the exposure time.
		status = dvpSetFloatValue(m_handle, V_EXPOSURE_TIME_F, fExpoTime);
		ASSERT(status == DVP_STATUS_OK);
...
void CBasicFunctionDlg::OnNMCustomdrawSliderExposureGain(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		status = dvpSetFloatValue(m_handle, V_GAIN_F, fGain);
		ASSERT(status == DVP_STATUS_OK);
...
void CBasicFunctionDlg::OnCbnSelchangeComboAeMode()
{
	...
	// get the auto exposure string by index
	dvpEnumDescr stEnumDescr;
	dvpGetEnumDescr(m_handle, V_EXPOSURE_AUTO_E, iSel, &stEnumDescr);

	// set the auto exposure.
	status = dvpSetEnumValueByString(m_handle, V_EXPOSURE_AUTO_E, stEnumDescr.szEnumName);
...
void CBasicFunctionDlg::OnCbnSelchangeComboFlick()
{
	...
	{
		//Disable
		status = dvpSetEnumValue(m_handle, V_ANTI_FLICK_E, 0);
	}
	else if (iSel == 1)
	{
		//AC_50Hz
		status = dvpSetEnumValue(m_handle, V_ANTI_FLICK_E, 1);
	}
	else if (iSel == 2)
	{
		//AC_60Hz
		status = dvpSetEnumValue(m_handle, V_ANTI_FLICK_E, 2);
	}

	ASSERT(status == DVP_STATUS_OK);
}

// Initial the auto white balance.
void CBasicFunctionDlg::InitAWB()
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		char szTmp[64];
		status = dvpGetEnumValueByString(m_handle, V_AWB_E, szTmp);
		// ASSERT(status == DVP_STATUS_OK);
		if (status != DVP_STATUS_OK)
		{
			// Auto white balance is not supported.
		...
		CheckDlgButton(IDC_CHECK_AWB, strcmp(szTmp, "Continuous") == 0 ? BST_CHECKED : BST_UNCHECKED);
		GetDlgItem(IDC_BUTTON_ONCE_AWB)->EnableWindow(strcmp(szTmp, "Continuous") == 0 ? FALSE : TRUE);
	}
	else
	{
		GetDlgItem(IDC_CHECK_AWB)->EnableWindow(false);
...
void CBasicFunctionDlg::OnBnClickedCheckAwb()
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		if (IsDlgButtonChecked(IDC_CHECK_AWB) == BST_CHECKED)
		{
			char szTmp[64];
			status = dvpGetEnumValueByString(m_handle, V_AWB_E, szTmp);
			if (!(strcmp(szTmp, "Continuous") == 0))
			{
				status = dvpSetEnumValueByString(m_handle, V_AWB_E, "Continuous");
				ASSERT(status == DVP_STATUS_OK);
			}
		}
		else
		{
			...
			status = dvpSetEnumValueByString(m_handle, V_AWB_E, "Disable");
			ASSERT(status == DVP_STATUS_OK);
		}
...
void CBasicFunctionDlg::OnBnClickedButtonOnceAwb()
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		status = dvpSetEnumValueByString(m_handle, V_AWB_E, "Once");
		ASSERT(status == DVP_STATUS_OK);
		...
		if (status == DVP_STATUS_OK)
		{
			...
			{
				//set rgb gain
				status = dvpSetFloatValue(m_handle, V_GAIN_R_F, 1.0f);
				//ASSERT(status == DVP_STATUS_OK);
				status = dvpSetFloatValue(m_handle, V_GAIN_G_F, 1.0f);
				//ASSERT(status == DVP_STATUS_OK);
				status = dvpSetFloatValue(m_handle, V_GAIN_B_F, 1.0f);
				//ASSERT(status == DVP_STATUS_OK);
			}
			else
			{
...
void CBasicFunctionDlg::OnBnClickedButtonExposureTime()
{
	TCHAR szTmp[128] = { 0 };
	GetDlgItemText(IDC_EDIT_EXPOSURE_TIME, szTmp, sizeof(szTmp) - 1);
	float f_time = (float)_ttof(szTmp);
	dvpStatus status;

	if (f_time > m_fMaxExposureTime - 1)
		f_time = (float)(m_fMaxExposureTime - 1);
	if (f_time < m_fMinExposureTime + 1)
		f_time = (float)(m_fMinExposureTime + 1);

	if (IsValidHandle(m_handle))
	{
		status = dvpSetFloatValue(m_handle, V_EXPOSURE_TIME_F, f_time);
		ASSERT(status == DVP_STATUS_OK);

		// Get the value of the exposure time again,there are differences between the set value and the obtained value for the reason of accuracy(step value),
		// it is subject to the obtained value.
		status = dvpGetFloatValue(m_handle, V_EXPOSURE_TIME_F, &f_time, NULL);
		ASSERT(status == DVP_STATUS_OK);
...
void CBasicFunctionDlg::OnBnClickedCheckUserdefinedname()
{
	...
	for (int i = 0; i < m_CamCount; i++)
	{
		...
		if (!m_bUserDefinedName)
		{
			item = m_listDevices.AddString(CStringW(m_info[i].FriendlyName));
			if (strName == CStringW(m_info[i].UserID))
			{
		...
		else
		{
			...
			item = m_listDevices.AddString(CStringW(m_info[i].UserID));
			if (strName == CStringW(m_info[i].FriendlyName))
			{
				m_listDevices.SetCurSel(item);
			}
		}
